...
async function getOrCreateProject(browser: Browser): Promise<string> {
  ...
  await page.locator('input').first().fill('pw-consult-' + Date.now());
...
test('S2-1: 顧問模式合議 toggle 預設為 ON', async ({ page, browser }) => {
  ...
  await page.goto(url);
  await page.locator('button:has-text("顧問"), [aria-pressed]').filter({ hasText: '顧問' }).click().catch(() => {});
...
test('S2-3: 送出問題後出現 phase 動畫（推理中）', async ({ page, browser }) => {
  const url = await getOrCreateProject(browser);
  await page.goto(url);
  await page.locator('button:has-text("顧問")').click().catch(() => {});
  const composer = page.locator('textarea').last();
  await composer.fill('你好');
  await composer.press('Enter');
  // Should see some phase indicator briefly
  const phase = page.locator('[class*="phase-indicator"], [class*="phase"]');
