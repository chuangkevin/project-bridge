...
describe('POST /api/projects/:id/ingest (M1 anonymous)', () => {
...
describe('GET /api/projects/:id/ingest (M1 anonymous)', () => {
  ...
  it('returns 404 for unknown project', async () => {
    const r = await request(app).get('/api/projects/no-such-project/ingest');
    expect(r.status).toBe(404);
  });
});
