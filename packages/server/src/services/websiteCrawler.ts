/**
 * websiteCrawler.ts — Playwright-based website design style extraction.
 *
 * crawlWebsite(url)        — single URL: extracts colors/typography/buttons/inputs/borders/shadows + screenshot
...
const EMPTY_RESULT = (url: string, error: string): CrawledStyles => ({
  url,
  success: false,
  error,
  colors: [],
  typography: { fonts: [], sizes: [], headings: [], body: null },
  buttons: [],
  inputs: [],
  backgrounds: [],
  borderRadii: [],
...
export async function crawlWebsite(url: string): Promise<CrawledStyles> {
  try {
    ...
    return EMPTY_RESULT(url, 'invalid_url');
  }
  ...
  try {
    ...
    if ((response && response.status() === 403) || looksForbiddenHtml(html)) {
      await context.close();
      return EMPTY_RESULT(url, 'forbidden');
    }
    ...
    if (err.message?.includes('timeout') || err.message?.includes('Timeout')) {
      return EMPTY_RESULT(url, 'timeout');
    }
    if (err.message?.includes('net::ERR_') || err.message?.includes('NS_ERROR_')) {
      return EMPTY_RESULT(url, 'blocked');
    }
    return EMPTY_RESULT(url, err.message?.slice(0, 200) || 'unknown');
  }
}
