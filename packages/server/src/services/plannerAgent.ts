...
function selectSkillsForRole(
  ...
  const result = scored.slice(0, 3).map(s => ({
    name: s.skill.name,
    content: s.skill.content.slice(0, 2000) + (s.skill.content.length > 2000 ? '...' : ''),
  }));
...
export async function planAndReview(
  ...
  const historyContext = history.length > 0
    ? `\n\n【先前對話】\n${history.slice(-5).map(h => `${h.role === 'user' ? '用戶' : '助手'}：${h.content.slice(0, 1000)}`).join('\n')}\n`
    : '';
...
精簡扼要，不超過 15 行。`, ux.name, 10000);
  ...
  try {
    ...
    const skillRulesList = qaSkills.length > 0
      ? `\n\n⚠️ 以下是專案知識庫（Skill）的業務規則，你必須逐條比對方案是否違反：\n${qaSkills.map(s => `• 【${s.name}】${s.content.slice(0, 1000)}`).join('\n')}\n`
      : '';
...
${qa.name}（QA 審查員）：${qaText.slice(0, 4000)}
