...
Return JSON only: {"html": N, "accessibility": N, "responsive": N, "consistency": N}
...
export async function scorePrototype(html: string, apiKey: string): Promise<QualityScore> {
  ...
  const scores = JSON.parse(text) as { html: number; accessibility: number; responsive: number; consistency: number };
  ...
  return {
    ...
    overall: clamp(Math.round((scores.html + scores.accessibility + scores.responsive + scores.consistency) / 4)),
  };
}
