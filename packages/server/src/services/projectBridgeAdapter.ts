 ...
 *
 * Ported from legacy v1.5 packages/server/src/services/projectBridgeAdapter.ts.
 * Import changes: removed top-level `db` singleton import (legacy used `../db/connection`
...
/** Module-level db reference set by initProjectBridgeAdapter(). */
let _db: Database.Database | null = null;

function getDb(): Database.Database {
  if (!_db) {
    throw new Error(
      "[projectBridgeAdapter] initProjectBridgeAdapter(db) must be called before using the adapter"
    );
  }
  return _db;
}
...
/**
 * Must be called from createApp() (or equivalent entry point) before
 * getProjectBridgeKeyPool() is used. Idempotent — safe to call again on
 * hot-reload if db instance is stable.
 */
...
export class ProjectBridgeAdapter implements StorageAdapter {
  ...
  async updateKey(key: ApiKey, expectedLeaseToken?: string | null): Promise<void> {
    ...
    if (!key.isActive) {
      ...
      db.prepare(
        "INSERT INTO settings (key, value) VALUES ('blocked_api_keys', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value"
      ).run([...blocked].join(","));
    }
  }
}
...
export function forceClearOldestAdapterCooldown(excludeKey = ''): string | null {
  ...
  try {
    getDb().prepare('DELETE FROM api_key_cooldowns WHERE api_key_suffix = ?').run(oldestKey.slice(-4));
  } catch {}
  return oldestKey;
}
