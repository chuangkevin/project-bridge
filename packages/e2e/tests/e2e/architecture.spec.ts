...
test.describe('架構圖功能', () => {
  ...
  test('新增頁面節點 → 驗證出現', async ({ page }) => {
    ...
    // With arch_data set, project starts in design mode; click 架構圖 tab
  ...
  test('展開元件列表 → 新增元件', async ({ page }) => {
    ...
    // 元件編輯對話框應出現（ComponentEditorModal）
    await expect(page.getByText('新增元件')).toBeVisible();

    // 填入名稱（placeholder: "例：搜尋按鈕"）
    await page.locator('input[placeholder="例：搜尋按鈕"]').fill('導航按鈕');
  ...
  test('新增多狀態元件（分頁標籤）', async ({ page }) => {
    ...
    // 填入名稱
    await page.locator('input[placeholder="例：搜尋按鈕"]').fill('主選單分頁');
