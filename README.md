...
| AI | ai-core MultiProviderClient v3.0.0（OpenAI 主 + Gemini fallback；SSE 串流） |
...
# v1.4.0 新增
OPENAI_OAUTH_CLIENT_ID=app_EMoamEEZ73f0CkXaXp7hrann   # OpenAI OAuth public client id（必填）
# OPENAI_OAUTH_REDIRECT_URI=https://<host>/api/openai-oauth/callback   # 不填用 PUBLIC_BASE_URL 推導
# PUBLIC_BASE_URL=https://designbridge.example.com    # OAuth redirect_uri 用得到
# OPENAI_API_KEY=sk-...                               # 不走 OAuth 時備援
```

## 部署 v1.4.0

### v1.4.0 重點變更
- **AI Provider 改為 ai-core MultiProviderClient**（`@kevinsisi/ai-core` v3.0.0，pin SHA `6b36218b2a8af01d397ff71a91fbafaf38f5d4ee`），預設 OpenAI 主路 → Gemini key-pool fallback。
...
#    必填：OPENAI_OAUTH_CLIENT_ID=app_EMoamEEZ73f0CkXaXp7hrann
