...
| AI | `@kevinsisi/ai-core` v3.4.1 `MultiProviderClient`（OpenCode primary with multi-server fallback → Gemini key-pool fallback；streaming uses OpenCode pseudo-stream first；OpenAI/Codex retained as configured fallback） |
...
PUBLIC_BASE_URL=https://designbridge.example.com                # 生產必填：使用者瀏覽器看到的外部 URL
                                                                #   會被烘焙進可下載的 OpenAI auth helper 當 token 上傳目標
                                                                #   缺這個 helper 會打 localhost 然後 ECONNREFUSED
...
OPENCODE_SERVERS=http://localhost:4096,http://opencode2:4096    # 選填：多 OpenCode server，依序嘗試
...
- **v1.5.1 收斂**：server pin 升到 `@kevinsisi/ai-core` v3.4.1，`services/provider.ts` 改為 text/chat/streaming 走 OpenCode-first；`opencode_servers` / `OPENCODE_SERVERS` 可設定多個 OpenCode endpoint，會先依序嘗試 OpenCode servers，再 fallback 到 Gemini pool / OpenAI-Codex final fallback。Gemini key 驗證與 Gemini-only vision helper 仍保留 direct SDK 例外。
...
# 2. 設定環境變數（packages/server/.env 或 docker-compose.yml）
#    必填：PUBLIC_BASE_URL=https://<your-host>（OAuth helper 烘焙用）
