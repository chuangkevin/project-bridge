...
| AI | `@kevinsisi/ai-core` v3.4.0 `MultiProviderClient`（non-stream generate: OpenCode primary → Gemini key-pool fallback；streaming calls require streaming-capable adapters；OpenAI/Codex retained as configured fallback） |
...
- **v1.5 收斂**：server pin 升到 `@kevinsisi/ai-core` v3.4.0，`services/provider.ts` 改為 non-stream generate 走 OpenCode-first、Gemini pool fallback、OpenAI/Codex final fallback；SSE streaming 仍受 ai-core capability selection 約束，OpenCode adapter 沒有 `streamContent` 時會選 streaming-capable provider。Gemini key 驗證與 Gemini-only vision helper 仍保留 direct SDK 例外。
